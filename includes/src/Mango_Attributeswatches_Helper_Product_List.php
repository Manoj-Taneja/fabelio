...
//            echo '<span style="display:none">after loading gallery ' . $execution_time . ' s.</span>';
                    ...
                    foreach ($_swatches as $product_id => $_option) {
                        ...
                        if ($_image_source == "child") {
                            $product = Mage::getModel('catalog/product')->load($product_id);
                            $_thumbnail = Mage::helper('catalog/image')->init($product, 'image')->resize($this->_images_width, $this->_images_height);
                        } elseif ($_image_source == "gallery") {
                            if (isset($_gallery_images[$_option["value"]]) && trim($_gallery_images[$_option["value"]])) {
                                $_thumbnail = Mage::helper('catalog/image')->init($_product, 'thumbnail', $_gallery_images[$_option["value"]])->resize($this->_images_width, $this->_images_height);
                            }
                        }
                ...
                if (!$_product_image) {
                    $_product_image = (string) Mage::helper('catalog/image')->init($_product, 'small_image')->resize($this->_images_width, $this->_images_height);
                }
                if (!$_product_url) {
...
//    echo '<span style="display:none">' . $execution_time . ' s.</span>';
            ...
            if ($_alternate_image_source && $_product->getData($_alternate_image_source) && $_product->getData($_alternate_image_source) !== "no_selection") {
                $_hover_image = (string) Mage::helper('catalog/image')->init($_product, $_alternate_image_source)->resize($this->_images_width, $this->_images_height);
            }
    ...
    public function getSettting($_value){
        ...
        if(isset($this->_settings[$_value])) return $this->_settings[$_value];
        return false;
    }
