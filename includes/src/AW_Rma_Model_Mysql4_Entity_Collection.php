...
{
    private $_isStatusNameJoined = false;
    private $_isReasonNameJoined = false;
    ...
    {
        if ($this->_isStatusNameJoined) {
            $_helper = Mage::helper('awrma');
            foreach ($this->getItems() as $_item) {
                $_item->setData('status_name', $_helper->__($_item->getData('status_name')));
            }
        }
    }
    ...
    public function joinStatusNames()
    {
        if (!$this->_isStatusNameJoined) {
            $this->getSelect()
                ->joinLeft(
                    array('s' => $this->getTable('awrma/entity_status')), 'main_table.status = s.id',
                    array('status_name' => 's.name')
                );
            $this->_isStatusNameJoined = true;
        }
    ...
    public function joinRequestNames()
    {
        $this->getSelect()
            ->joinLeft(
                array('r' => $this->getTable('awrma/entity_types')), 'main_table.request_type = r.id',
                array('request_name' => 'r.name')
            );
    ...
    public function joinOrderStore()
    {
        $this->getSelect()
            ->joinLeft(
                array('o' => $this->getTable('sales/order')), 'main_table.order_id = o.increment_id',
    ...
    public function joinReasonNames()
    {
        if (!$this->_isReasonNameJoined) {
            $defaultValue = Mage::helper('awrma')->__('Other');
            $this->getSelect()
                ->joinLeft(
                    array('r' => $this->getTable('awrma/entity_reason')), 'main_table.reason_id = r.id',
                    array('reason_name' => "IF(`r`.`id`,`r`.`name`,'{$defaultValue}')")
                );
            $this->_isReasonNameJoined = true;
        }
