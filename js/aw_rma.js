var AWRMAOrdersControl = Class.create({
    ...
    initialize: function(globalObject, awrmaNewForm, getItemsUrl) {
        ...
        this.itemsSelectedClass = '.awrma-items-selected';
        /* Id of select/unselect all items toggle */
        this.itemsSelectToggle = 'awrma-items-select-toggle';
        /* Order item row id */
            ...
            onSuccess: function(transport) {
                ...
                if(typeof items == 'object') {
                    $(items).each(function(row) {
                        $(awrmaoco.orderItemsContainer).insert({
                            bottom: row
                        });
                    });
                    $(awrmaoco.orderSelectId).enable();
                    awrmaoco.observeItemsCount();
                    awrmaoco.observeSelectAllToggle();
                    awrmaoco.syncItems();
...
var AWRMAAdminRmaFormControl = Class.create({
    ...
    validateForm: function() {
        ...
        if(!this.form || !this.form.validator || !this.form.validator.validate()) {
            ...
            if(typeof(awrma_tabsJsTabs.tabs[1]) != 'undefined')
                awrma_tabsJsTabs.tabs[1].addClassName('error');
            return false;
        }
        if(chPassed) {
            if(typeof(awrma_tabsJsTabs.tabs[1]) != 'undefined')
                awrma_tabsJsTabs.tabs[1].removeClassName('error');
            return this.form.submit();
        } else {
            if(typeof(awrma_tabsJsTabs.tabs[1]) != 'undefined')
                awrma_tabsJsTabs.tabs[1].addClassName('error');
