...
class Veritrans_Vtwebmandiri_PaymentController
  ...
  public function redirectAction() {
    ...
    $enable_installment = Mage::getStoreConfig('payment/vtwebmandiri/enable_installment');
    $is_enabled_mandiri = Mage::getStoreConfig('payment/vtwebmandiri/enable_installment');
    // $is_enabled_mandiri = Mage::getStoreConfig('payment/vtwebmandiri/enable_installment_mandiri');
    ...
    $totalPrice = 0;
    $threshold = Mage::getStoreConfig('payment/vtwebmandiri/threshold'); //debugan
    ...
    $bin_list = Mage::getStoreConfig('payment/vtwebmandiri/bin_number_list'); 
    if($bin_list)
    {
        ...
        if (array_key_exists('attributes_info', $productOptions)) {
          foreach ($productOptions['attributes_info'] as $attribute) {
            if (in_array('Payment', $attribute)) {
              ...
              // Mage::log('value:'.print_r($attribute['value'],true),null,'vtwebmandiri.log',true); //debugan
              // Mage::log('explodenya:'.print_r($installment_value,true),null,'vtwebmandiri.log',true); //debugan
              // Mage::log('installment_value[0]:'.print_r($installment_value[0],true),null,'vtwebmandiri.log',true); //debugan

              if (strtolower($installment_value[0]) == 'installment') {

                // Mage::log('installement value:'.print_r($installment_value,true),null,'vtwebmandiri.log',true); //debugan
                // Mage::log('strtolower($installment_value[1]):'.print_r(strtolower($installment_value[1]),true),null,'vtwebmandiri.log',true); //debugan
                // Mage::log('strtolower($installment_value[2]):'.print_r(strtolower($installment_value[2]),true),null,'vtwebmandiri.log',true); //debugan
