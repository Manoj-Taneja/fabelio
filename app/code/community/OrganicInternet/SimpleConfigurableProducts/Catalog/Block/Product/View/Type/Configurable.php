    ...
    public function getJsonConfig()
    {
        ...
        foreach ($this->getAllowProducts() as $product) {
            ...
            $childProducts[$productId] = array(
                ...
                "rrp" => $this->_registerJsPrice($this->_convertPrice($product->getData('msrp')))
            );
            if ($product->getData('msrp')) {
              $childProducts[$productId]["rrp"] = Mage::helper('core')->currency($product->getData('msrp'), true, false);
            }
            ...
            $childProducts[$productId]["tabs"] = $childBlock->setTemplate('catalog/product/view/tabs.phtml')
              ->setProduct($product)
              ->toHtml();

            $childProducts[$productId]["imageZoomer"] = $childBlock->setTemplate('attributeswatches/catalog_product_view_media_static.phtml')
                ->setProduct($product)
                ->toHtml();
        ...
        $p = $this->getProduct();
        $config["tabs"] = $childBlock->setTemplate('catalog/product/view/tabs.phtml')
          ->setProduct($p)
          ->toHtml();
        ...
        $config['rrp'] = Mage::helper('core')->currency($p->getData('msrp'), true, false);
        ...
        if (Mage::getStoreConfig('SCP_options/product_page/change_image')) {
            if (Mage::getStoreConfig('SCP_options/product_page/change_image_fancy')) {
                ...
                $config["imageZoomer"] = $childBlock->setTemplate('attributeswatches/catalog_product_view_media_static.phtml')
                    ->setProduct($this->getProduct())
                    ->toHtml();
            }
        }
