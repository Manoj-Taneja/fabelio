...
class AW_Rma_Block_Adminhtml_Types_Edit_Form extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm()
    {
        ...
        if (Mage::getSingleton('adminhtml/session')->getAWRMATypesFormData()) {
            $data = Mage::getSingleton('adminhtml/session')->getAWRMATypesFormData(TRUE);
        }

        $typesSaveUrl = $this->getUrl(
            'awrma_admin/adminhtml_types/save', array('id' => $this->getRequest()->getParam('id'))
        );
        ...
        $_fieldset = $_form->addFieldset('type_fieldset', array( 'legend' => $this->__('Type Information')));
        $_fieldset->addField(
            'name', 'text',
            array(
                'name'     => 'name',
