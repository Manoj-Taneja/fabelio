...
class AW_Rma_Helper_Comments extends Mage_Core_Helper_Abstract
{
    ...
    public static function parseOrderId($text)
    {
        ...
        if (isset($matches[0])) {
            foreach ($matches[0] as $orderId) {
                ...
                if ($_order->getId()) {
                    $orderViewUrl = Mage::app()->getStore()->getUrl(
                        'sales/order/view', array('order_id' => $_order->getId())
                    );
                    $text = str_replace(
                        $orderIdInText,
                        "<a href=\"" . $orderViewUrl . "\">{$orderIdInText}</a>",
                        $text
                    );
                }
            }
    ...
    public static function postComment($id, $text, $data = array(), $sendNotify = true)
    {
        if ($id) {
            ...
            if ($sendNotify) {
                Mage::getModel('awrma/notify')->notifyAboutComment(
                    $id, $_comment, $data['owner'] == AW_Rma_Model_Source_Owner::CUSTOMER
                );
            }
            ...
            if (!in_array($_rmaRequest->getStatus(), Mage::helper('awrma/status')->getResolvedStatuses())) {
                ...
                if ($_data['text']) {
                    if (isset($_FILES['attachedfile']['name']) && $_FILES['attachedfile']['name']) {
                        $fileExtension = Mage::helper('awrma/files')->getExtension($_FILES['attachedfile']['name']);
                        if (!in_array($fileExtension, Mage::helper('awrma/config')->getForbiddenExtensions())) {
                            if ($_FILES['attachedfile']['size'] <= Mage::helper('awrma/config')->getMaxAttachmentsSize()
                                && $_FILES['attachedfile']['size'] > 0) {
                                if ($_FILES['attachedfile']['error'] == UPLOAD_ERR_OK) {
                                    try {
                                        ...
                                        $result = $uploader->save(
                                            Mage::helper('awrma/files')->getPath(), $_FILES['attachedfile']['name']
                                        );
