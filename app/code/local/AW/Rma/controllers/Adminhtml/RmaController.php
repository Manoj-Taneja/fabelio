...
class AW_Rma_Adminhtml_RmaController extends AW_Rma_Adminhtml_AbstractController
{
    ...
    protected function saveAction()
    {
        ...
        $_data['rma_id'] = trim($this->getRequest()->getParam('rma_id'));
        ...
        if (Mage::helper('awrma/config')->getReasonsEnabled()) {
            $_data['reason_id'] = $this->getRequest()->getParam('reason_id');
            if (Mage::helper('awrma/config')->getReasonsOtherOptionEnabled()) {
                $_data['reason_details'] = $this->getRequest()->getParam('reason_details');
            }
        }
        ...
        foreach ($_orderItems as $_item) {
            if ($_item->getData('product_type') == 'bundle' && $_item->getChildrenItems()) {
                ...
                foreach ($_item->getChildrenItems() as $bundleItem) {
                    if (isset($_data['order_items'][$bundleItem->getData('item_id')])) {
                        $_data['order_items'][$_item->getData('item_id')] +=
                            $_data['order_items'][$bundleItem->getData('item_id')]
                        ;
                    }
                }
            }
            ...
            if ($this->getRequest()->getParam('comment_text')) {
                ...
                if ($_data['text']) {
                    if (isset($_FILES['comment_file']['name']) && $_FILES['comment_file']['name']) {
                        $fileExtension = Mage::helper('awrma/files')->getExtension($_FILES['comment_file']['name']);
                        if (!in_array($fileExtension, Mage::helper('awrma/config')->getForbiddenExtensions())) {
                            if ($_FILES['comment_file']['size'] <= Mage::helper('awrma/config')->getMaxAttachmentsSize()
                                && $_FILES['comment_file']['size'] > 0 ) {
                                if ($_FILES['comment_file']['error'] == UPLOAD_ERR_OK) {
                                    try {
                                        ...
                                        $result = $uploader->save(
                                            Mage::helper('awrma/files')->getPath(), $_FILES['comment_file']['name']
                                        );
                                    ...
                                    $this->_getSession()->addError(
                                        Mage::helper('awrma')->__('Some error occurs when uploading file')
                                    );
                                }
                            } else {
                                $this->_getSession()->addError(
                                    Mage::helper('awrma')->__(
                                        'Maximal allowed attachment size is '
                                        . (floor(Mage::helper('awrma/config')->getMaxAttachmentsSize() / 1024))
                                        . ' kb'
                                    )
                                );
                            }
                    ...
                    if (!$this->hasErrors()) {
                        ...
                        $comment = Mage::helper('awrma/comments')->postComment(
                            $rmaRequest->getId(), $_data['text'], $_data, false
                        );
                        $this->_getSession()->addSuccess(Mage::helper('awrma')->__('Comment successfully added'));
                        Mage::getModel('awrma/notify')->checkChanges($rmaRequest, false, $comment);
                        $_notified = true;
                    }
                } else {
        ...
        if ($this->hasErrors()) {
            ...
            if (!$_notified) {
                Mage::getModel('awrma/notify')->checkChanges($rmaRequest, false);
            }
            if ($this->getRequest()->getParam('continue')) {
                ...
                return $this->_redirect(
                    ...
                    array(
                         'id' => $rmaRequest->getId(),
                         'printstore' => $rmaRequest->getStoreId(),
                         'printurl' => $rmaRequest->getExternalLink()
                    )
                );
            } else {
                return $this->_redirect('*/*/' . ($listType ? $listType : 'list'));
    ...
    protected function _isAllowed()
    {
        ...
        if (Mage::getSingleton('admin/session')->getAcl()->has('sales/awrma/' . $_action)) {
                ...
                if ($_addNewEntityFlag) {
                    ...
                    if (!($packageOpenedOption === false)) {
                        ...
                        if ($_order->getCustomerIsGuest()) {
                            $_data['customer_name'] = $_order->getBillingAddress()->getName();
                        } else {
    ...
    protected function ordergridAction()
    {
        ...
        $this->getResponse()->setBody(
            $this->getLayout()->createBlock('awrma/adminhtml_sales_order_view_tabs_requests_grid')->toHtml()
        );
    }

    protected function customergridAction()
    {
        $this->loadLayout();

        $id = intval($this->getRequest()->getParam('customer_id'));
    ...
    {
        $fileName = 'rma_requests.csv';
        $content = $this->getLayout()->createBlock('awrma/adminhtml_rma_grid')
            ->getCsv();
