 ...
 */
abstract class Kellton_Checkout_Block_Onepage_Abstract extends Mage_Core_Block_Template
{
    protected $_customer;
    protected $_checkout;
    protected $_quote;
    ...
    public function getCountryCollection($type='billing')
    {
        if (!$this->_countryCollection) {
            $this->_countryCollection = Mage::getSingleton('directory/country')->getResourceCollection();
            if(Mage::helper('core')->isModuleEnabled('Mexbs_Shipping')){
                $this->_countryCollection->setAllowedType($type);
            }
    ...
    public function getCountryHtmlSelect($type)
    {
        ...
        $select = $this->getLayout()->createBlock('core/html_select')
            ...
            ->setValue($countryId)
            ->setOptions($this->getCountryOptions($type));
        if ($type === 'shipping') {
    ...
    public function getCountryOptions($type='billing')
    {
        $options    = false;
        $useCache   = Mage::app()->useCache('config');
        if ($useCache) {
            $cacheId    = 'DIRECTORY_COUNTRY_SELECT_STORE_' . $type . Mage::app()->getStore()->getCode();
