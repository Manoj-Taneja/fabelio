...
class Mage_Catalog_Model_Layer extends Varien_Object
{
       ...
       $parentIds = Mage::getResourceSingleton('catalog/product_type_configurable')->getParentIdsByChild($_product->getId());
        $idTocheck=0;
                if(count($parentIds)>0){
                    $idTocheck=$parentIds[0];
                    $childProductIds[]=$_product->getId();
                    $childCount[$idTocheck]=$childCount[$idTocheck]+1;
                    
                }else{
                    $idTocheck=$_product->getId();
                    if($_product->getTypeId() == 'configurable') {
                        if(array_key_exists($idTocheck, $childCount)){
                            
                        }else{
                             $childCount[$idTocheck]=0;
                        }
                    }
                    
                }
                if(in_array($idTocheck, $idsAdded)){
                    
                    
                    continue;
                }else{
                    if($idTocheck==$_product->getId()){
                        $idsAdded[]=$idTocheck;
                    }else{
                         continue;
                    }
    ...
    $collection = $this->getCurrentCategory()->getProductCollection()->addAttributeToFilter('entity_id', $idsAdded);
