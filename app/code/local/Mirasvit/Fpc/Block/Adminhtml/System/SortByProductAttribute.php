...
class Mirasvit_Fpc_Block_Adminhtml_System_SortByProductAttribute extends Mage_Adminhtml_Block_System_Config_Form_Field_Array_Abstract
{
    ...
    protected function _getOptionsRenderer()
    {
        if (!$this->_optionsRenderer) {
            $this->_optionsRenderer = Mage::app()->getLayout()->createBlock(
                'fpc/adminhtml_system_attributeOption', '',
                array('is_render_to_js_template' => true)
            );
            $this->_optionsRenderer->setClass('customer_options_select');
            $this->_optionsRenderer->setExtraParams('style="width:100px"');
        }
    ...
    public function __construct()
    {
        ...
        $this->addColumn('attribute_option_code', array(
            'label' => __('Attribute'),
            'renderer' => $select,
        ));
    ...
    {
        $row->setData(
            'option_extra_attr_'.$this->_getOptionsRenderer()->calcOptionHash($row->getData('attribute_option_code')),
            'selected="selected"'
        );
    }
