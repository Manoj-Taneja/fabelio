...
//    custom block 
    ...
    public function drawMenuItem($children, $level = 1, $type, $width, $mode = 'dt') {
      ...
      $catProdCounts = $this->_getEnabledProductsCountData();
      $html = '';        
      foreach ($children as $child) {
        if (is_object($child) && $child->getIsActive()) {
          ...
          if ($level == 1) {
            ...
            $html .= '<li class="'.$class.' '.$active.'">';
            $html .= '<a class="level' . $level . '" href="' . $this->getCategoryUrl($child) . '"><span>';
            $html .= $name . $label;
            $html .= ' ('. $catProdCounts[$id]['product_count']. ')';
            $html .= '</span></a>';
          }
          if (count($activeChildren) > 0) {
            if ($mode != 'mb') {
...
//                    $html .= '<div class="clearfix"></div>';
    ...
    private function _getEnabledProductsCountData()
    {
        if (is_null($this->_enabledProductsCount))
        {
            $collection = Mage::getModel('catalog/category')->getCollection();
            $storeId = Mage::app()->getStore()->getId();
            ...
            #Mage::log($productsCount);
            $this->_enabledProductsCount = $productsCount;
        }
        return $this->_enabledProductsCount;
    }
