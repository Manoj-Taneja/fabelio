...
class Smartwave_Blog_Model_Blog extends Mage_Core_Model_Abstract
{
    ...
    public function getFormatted($pattern, $date = false) {

        $orig_data = $this->getOrigData();
        $oldLocale = setlocale(LC_COLLATE, "0");

        setlocale(LC_ALL, Mage::app()->getLocale()->getLocaleCode().'.UTF-8');
        $formatted = strftime($pattern, strtotime($date? $date : $orig_data['created_time']) + Mage::getSingleton('core/date')->getGmtOffset());
