 ...
 * @Class        Ptech_Multilayer_Block_Layer_View
 * @Overwrite    Mage_Catalog_Block_Layer_View
 */
class Ptech_Multilayer_Block_Layer_View extends Mage_Catalog_Block_Layer_View {
    ...
    public function getStateInfo() { 
        ...
        $ajaxUrl = '';
        if ($_hlp->isSearch()) {
            $ajaxUrl = Mage::getUrl('multilayer/front/search');
        } elseif ($cat = $this->getLayer()->getCurrentCategory()) {
            $ajaxUrl = Mage::getUrl('multilayer/front/category', array('id' => $cat->getId()));
        }
    ...
    protected function _prepareLayout() {
        ...
        foreach ($filterableAttributes as $attribute) {
            $blockType = 'multilayer/layer_filter_attribute';

            if ($attribute->getFrontendInput() == 'price') {
                $blockType = 'multilayer/layer_filter_price';
            }
    ...
    protected function createCategoriesBlock() {
        ...
        if ('none' != $_hlp->catStyle()) {
            $categoryBlock = $this->getLayout()->createBlock('multilayer/layer_filter_category')
                    ->setLayer($this->getLayer())
                    ->init();
