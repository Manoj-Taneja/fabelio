 ...
 * @Class        Ptech_Multilayer_Helper_Data
 * @Overwrite    Mage_Core_Helper_Abstract
 */
class Ptech_Multilayer_Helper_Data extends Mage_Core_Helper_Abstract {

    protected $_params = null;
    protected $_continueShoppingUrl = null;

    const XML_PATH_CAT_STYLE = 'multilayer/multilayer/cat_style';
    const XML_PATH_REMOVE_LINKS = 'multilayer/multilayer/remove_links';
    const XML_PATH_RESET_FILTERS = 'multilayer/multilayer/reset_filters';
    ...
    public function isSearch() {
        ...
        if ('multilayer' === $mod && 'search' == Mage::app()->getRequest()->getActionName()) {
            return true;
        }
    ...
    public function wrapProducts($html) {
        ...
        $loaderHtml = '<div class="ptech_loading_filters" style="display:none"><img id="loading-image" src="' . Mage::getDesign()->getSkinUrl('images/Ptech/ajax-loader.gif') . '" /></div>';
        $html .= $loaderHtml;
        ...
        if (Mage::app()->getRequest()->isXmlHttpRequest()) {
            ...
            $html = '<div id="ptech_multilayer_container">'
                    . $html
                    . '</div>'
                    . '';
        }
    ...
    public function getParams($asString = false, $without = null) {
        if (is_null($this->_params)) {
            ...
            $nCurrentPriceStyle = Mage::getStoreConfig('multilayer/multilayer/price_style');
