...
class Amasty_Xlanding_Model_Page extends Mage_Rule_Model_Rule
{
    ...
    public function getLandingUsps(){
      $landingUsps = $this->getData('landing_usps');
      return $landingUsps? explode(',', $landingUsps) : array();
    }
...
//            'amlanding_cp.product_id = e.entity_id AND amlanding_cp.category_id IN (' . implode(',', $deepCategories) . ')'
    ...
    public function getLayoutFileMobileUrl(){
        return Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . DS . $this->getLayoutFileMobile();
    }
    ...
    protected function _beforeSave(){
        ...
        if(isset($_FILES['layout_file_mobile']) && $_FILES['layout_file_mobile']['name'] != '') {
            try{
                $uploader = new Varien_File_Uploader('layout_file_mobile');
                $uploader->setAllowedExtensions(array('jpg','jpeg','gif','png'));
                $uploader->setAllowRenameFiles(true);
                $uploader->setFilesDispersion(true);

                $this->setLayoutFileMobileName($uploader->getCorrectFileName($_FILES['layout_file_mobile']['name']));
                $ext = pathinfo($_FILES['layout_file_mobile']['name'], PATHINFO_EXTENSION);

                $result = $uploader->save(
                    Mage::getBaseDir('media') . DS . $this->getUploadPath(), uniqid().".".$ext
                );

                $this->setLayoutFileMobile($this->getUploadPath() . $result['file']); 
            } catch (Exception $e){
                Mage::throwException($this->__('Invalid image format'));
            }
            ...
            $layoutFile = $this->getLayoutFileMobile();
            
            if (isset($layoutFile['delete']) && $layoutFile['delete'] == 1) {
                $this->setLayoutFileMobile(NULL);
            } else {
                $this->setLayoutFileMobile($this->layout_file_mobile["value"]);
            }
            
        }

        $this->setLandingUsps(implode(',', $_POST['landing_usps']));
