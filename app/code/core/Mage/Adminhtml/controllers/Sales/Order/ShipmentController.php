    ...
    public function saveAction()
    {
        ...
        try {
            ...
            $select = $connection->select()
                ->from('sales_flat_order', array('cbpa', 'grand_total'))
                ->where('entity_id=?', $this->getRequest()->getParam('order_id'));
            $rowArray =$connection->fetchRow($select);   //return row
            $valCPS = $rowArray['grand_total'] * (3/100);

            /* end select */


            @session_start();
            if(!empty($rowArray['cbpa'])){
                $idOrder = $this->getRequest()->getParam('order_id');
                /* Mengambil Value Cookie */
                $idTrans = $rowArray['cbpa'];

                /* Menghapus Cookie */
                setcookie("priceareacps_id", "", time() - (86400*30));
                ...
                curl_setopt($ch, CURLOPT_POSTFIELDS, "app_key=".$apiKEY."&app_id=".$apiAPP."&value=".$idOrder."&cashback=".$valCPS."&idtracking=".$idTrans."&action=selesai");
